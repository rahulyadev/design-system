...
The packed preview uses the fixed nonce `design-system-preview-nonce` for deterministic local automation only. Production responses must use a fresh unpredictable nonce or an exact CSP hash for the stable bootstrap text. Do not reuse the preview nonce in a deployed consumer.
...
Public modules do not access DOM or browser storage during module evaluation. The provider uses a stable `system:light` server snapshot. The head bootstrap performs client first-paint selection. Local packed-preview automation covers six first-paint storage and system combinations, hydration without mismatch warnings, system changes, and matching cross-tab storage events in the tested browser matrix. These results do not establish equivalence in an untested consumer shell.
